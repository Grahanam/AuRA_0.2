...
import {faMusic,faClock,faHashtag,faSearch,faPlus,faX, faDumpster, faDeleteLeft, faMinus} from "@fortawesome/free-solid-svg-icons";
        ...
        {playlist.tracks.map((song,index) => (
          <li key={index}>
            <div className={`flex flex-row items-center align-center ${track._id==song._id?'text-green-500':'text-white'} justify-center  py-2 p-6`}>
              ...
              <div className="w-2/12">
                <h1 className="text-xl font-medium tracking-wide">{Math.floor(song.duration/60)}:{song.duration%60<10?'0':''}{song.duration%60}</h1>
              </div>
              <div className="flex p-2 md:p-4 lg:p-4 items-start justify-self-end ">
                
                {track._id==song._id?<i><FontAwesomeIcon icon={faMinus} className="mt-2 text-l"/></i>
                :<>
                    <i onClick={()=>removetrack(song._id)}><FontAwesomeIcon icon={faX} className="text-white text-m "/></i>
                </>}
              </div>
            </div>
          </li>
