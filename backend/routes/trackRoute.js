...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let picturefileName = `aura/track_img/${Date.now()}_${
        picture.originalname
      }`;
      let audiofileName = `aura/track_audio/${Date.now()}_${
        audio.originalname
      }`;
    ...
    try {
      ...
      if (!req.file) {
        ...
        if (req.files.picture[0]) {
          ...
          const filename = `aura/track_img/${Date.now()}_${
            picture.originalname
          }`;
          const uuid = UUID();
        ...
        if (req.files.audio[0]) {
          ...
          const filename = `aura/track_audio/${Date.now()}_${
            audio.originalname
          }`;
          const uuid = UUID();
          const fileURL = await uploadFile(
        ...
        const updatetrack = await trackModel.findByIdAndUpdate(
          track._id,
          track,
          { new: true }
        );
